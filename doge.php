...
error_reporting(0);
function call($addr) {
    $data= "address=$addr&sponsor=1930";
    $cok = tempnam('tmp','avo'.rand(10000000).'tmp.txt');
    $c = curl_init("http://cryptofun.space/?id=1930");
    curl_setopt($c, CURLOPT_REFERER, "http://cryptofun.space/?id=1930");
 ...
 $data= "faucetclaim=$addr";
    ...
    preg_match_all('~(<h2 style="color:#4dce22"><b>(.*?)</h2>)~', $body, $bal);
...
for($i=0; $i<100; $i++) {
    ...
    echo "[" . date("H:i:s") . "] Mohon Tunggu 15 menit...\n";
    sleep(900);
}